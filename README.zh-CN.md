...
- **种子详情** —— 常规、Tracker、连接、内容、HTTP 源 五个标签页。连接页会安全显示规范的
  I2P 节点地址;内容可按名称/大小/进度/优先级排序。选择任一列时默认升序,重复点击当前列则切换
  方向,与 qui 当前规则一致。
- **仪表盘** —— qui 的全局统计卡片、可折叠的服务器统计卡片、跨客户端合并的 Tracker 明细,
  以及每个客户端的卡片(数量、传输总量、磁盘占用、备用速度限制开关)。显示哪些板块可在设置里
  配置。
...
**设置 → 小部件 → 后台刷新**可以选间隔:关闭、15、30 或 60 分钟。15 分钟是安卓对周期性后台
任务强制的下限。平台自带的小部件更新计划最低只到 30 分钟且无法调低,所以小部件不再使用它——
`updatePeriodMillis` 设为 0,改由 WorkManager 任务驱动。

不管间隔设成多少,这两件事都会立刻更新小部件:

- 打开应用 —— 每次仪表盘轮询都会把新数字推给所有已放置的小部件;
- 小部件自己标题栏上的刷新按钮。

刷新不再在广播接收器里执行。接收器大约只有十秒就会被系统判定为卡死,而它唤醒的进程可能是冷启动的
—— Hilt 依赖图、读 DataStore、DNS、TLS 握手,然后才是请求。一台一天没打开过应用的手机根本跑不完,
这就是为什么一台完全能连上的服务器会被报成"无法连接",而且点刷新永远没用。现在接收器只画一个
**刷新中…**的状态,把请求交给一个没有时限的后台任务:它会等网络就绪而不是没网就失败,并且会先重试
一次再把结果当成错误显示出来。

真的失败时,小部件会说清楚原因——没有网络、凭据被拒绝、服务器超时、HTTP 502——而不是一律归咎于
服务器。

在小米设备上,**设置 → 应用设置 → qui → 自启动**打开、**省电策略 → 无限制**,后台任务才跑得起来。
不设置的话,小部件会一直显示它最后一次成功获取到的数字(带时间戳),直到你点刷新。
...
- **小部件的后台刷新**不能低于 15 分钟;这是安卓对周期性任务的硬性下限,不是设计取舍——见[刷新](#刷新)。
