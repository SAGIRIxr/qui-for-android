...
- **Torrent details** — General, Trackers, Peers, Content and HTTP Sources tabs. Peers
  show canonical `.b32.i2p` addresses for I2P connections, and Content can be sorted by
  name, size, progress or priority. Each column starts ascending when selected; tapping
  the active column toggles direction, matching qui's current rule.
- **Dashboard** — qui's global stat tiles, a collapsible Server Statistics card, the
  tracker breakdown merged across clients, and per-client cards with counts, transfer
  totals, disk usage and the alternative-speed switch. Which sections appear is
  configurable.
